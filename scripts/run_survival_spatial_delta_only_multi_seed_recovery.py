...
class MultiSeedSpatialDeltaOnlyRecoveryConfig:
    ...
    out_dir: str = "artifacts/survival_spatial_delta_only_multi_seed_recovery"
...
def _extract_truth(wide: pd.DataFrame) -> tuple[dict[str, float], pd.DataFrame]:
    ...
    param_truth = {
        ...
        # delta-only parameterization: these are full post-treatment effects
        "delta_post[0]": beta_td_true + float(first["delta_post_0_true"]),
        "delta_post[1]": beta_td_true + float(first["delta_post_1_true"]),
        "delta_post[2]": beta_td_true + float(first["delta_post_2_true"]),
        "delta_post[3]": beta_td_true + float(first["delta_post_3_true"]),
        "delta_post[4]": beta_td_true + float(first["delta_post_4_true"]),
...
def _one_seed_recovery(
    seed: int,
    cfg: MultiSeedSpatialDeltaOnlyRecoveryConfig,
...
def main(config: MultiSeedSpatialDeltaOnlyRecoveryConfig | None = None) -> None:
    cfg = MultiSeedSpatialDeltaOnlyRecoveryConfig() if config is None else config
    ...
    print("\nRunning multi-seed survival_spatial delta-only recovery study")
    print("=============================================================")
    ...
    for i, seed in enumerate(cfg.seeds, start=1):
        ...
        print(
            ...
            f"rmse_u={seed_meta['rmse_u_true_vs_post_mean']:.4f}, "
            f"param_mae={float(param_df['abs_error'].mean()):.4f}"
        )
