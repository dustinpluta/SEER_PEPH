...
class SpatialDeltaOnlyRecoveryConfig:
    seed: int = 123
    A: int = 20
    k: int = 4
    n_per_area: int = 200
    ...
    out_dir: str = "artifacts/survival_spatial_delta_only_recovery"
...
def main(config: SpatialDeltaOnlyRecoveryConfig | None = None) -> None:
    cfg = SpatialDeltaOnlyRecoveryConfig() if config is None else config
    ...
    print("\nSpatial survival delta-only recovery check")
    print("------------------------------------------")
    ...
    print("\nPost-treatment interval diagnostics")
    print("-----------------------------------")
    if len(interval_diag) > 0:
        print(interval_diag.to_string(index=False))
    else:
        print("(no treated rows)")
