...
from seer_peph.fitting.fit_models import JointFit
...
def joint_survival_ppc_interval_counts(
    fit: JointFit,
    ...
    *,
    ...
    draw_matrix = _joint_survival_rowwise_expected_counts(
        fit,
        df,
...
def joint_survival_ppc_area_counts(
    fit: JointFit,
    surv_long: pd.DataFrame,
    *,
    draw_indices: Sequence[int] | None = None,
...
def joint_survival_ppc_interval_by_treatment_counts(
    fit: JointFit,
    surv_long: pd.DataFrame,
    *,
    draw_indices: Sequence[int] | None = None,
...
def joint_treatment_ppc_interval_counts(
    fit: JointFit,
    ...
    *,
    ...
    draw_matrix = _joint_treatment_rowwise_expected_counts(
        fit,
        df,
        draw_indices=draw_indices,
    )
...
def joint_treatment_ppc_area_counts(
    fit: JointFit,
    ttt_long: pd.DataFrame,
    *,
    draw_indices: Sequence[int] | None = None,
...
# -----------------------------------------------------------------------------
# Rowwise expectations
# -----------------------------------------------------------------------------

def joint_survival_ppc_row_expectations(
    fit: JointFit,
    surv_long: pd.DataFrame,
    *,
    draw_indices: Sequence[int] | None = None,
...
def joint_treatment_ppc_row_expectations(
    fit: JointFit,
    ttt_long: pd.DataFrame,
    *,
    draw_indices: Sequence[int] | None = None,
...
def _prepare_surv_long(fit: JointFit, surv_long: pd.DataFrame) -> pd.DataFrame:
...
def _prepare_ttt_long(fit: JointFit, ttt_long: pd.DataFrame) -> pd.DataFrame:
...
def _joint_survival_rowwise_expected_counts(
    fit: JointFit,
    surv_long: pd.DataFrame,
    *,
    ...
    # Prefer the structured survival field if present; otherwise fall back to u.
    if "u_surv" in samples:
        u_surv = np.asarray(samples["u_surv"], dtype=float)
    elif "u" in samples:
        u_surv = np.asarray(samples["u"], dtype=float)
    else:
        raise ValueError("Joint fit samples do not contain u_surv or u for survival PPC.")
    ...
    u_surv = u_surv[draw_idx]

    x_cols = list(fit.metadata.surv_x_cols or ())
    X = surv_long[x_cols].to_numpy(dtype=float)
    k = surv_long["k"].to_numpy(dtype=int)
    area_id = surv_long["area_id"].to_numpy(dtype=int)
    treated_td = surv_long["treated_td"].to_numpy(dtype=float)
    k_post = surv_long["k_post"].to_numpy(dtype=int)
    exposure = surv_long["exposure"].to_numpy(dtype=float)

    _validate_survival_shapes(
        alpha=alpha,
        beta=beta,
...
def _joint_treatment_rowwise_expected_counts(
    fit: JointFit,
    ttt_long: pd.DataFrame,
    *,
    draw_indices: Sequence[int] | None,
    ...
    # Prefer structured treatment field if present; otherwise fall back to u.
    if "u_ttt" in samples:
        u_ttt = np.asarray(samples["u_ttt"], dtype=float)
    elif "u" in samples:
        u_ttt = np.asarray(samples["u"], dtype=float)
    else:
        raise ValueError("Joint fit samples do not contain u_ttt or u for treatment PPC.")
    ...
    u_ttt = u_ttt[draw_idx]

    x_cols = list(fit.metadata.ttt_x_cols or ())
    X = ttt_long[x_cols].to_numpy(dtype=float)
    k = ttt_long["k"].to_numpy(dtype=int)
    area_id = ttt_long["area_id"].to_numpy(dtype=int)
    exposure = ttt_long["exposure"].to_numpy(dtype=float)

    _validate_treatment_shapes(
        theta=theta,
        gamma=gamma,
        u=u_ttt,
        X=X,
...
def _validate_survival_shapes(
    *,
    ...
    if beta.shape[0] != n_draws or delta_post.shape[0] != n_draws or u.shape[0] != n_draws:
        raise ValueError("All survival sample blocks must have the same number of draws.")
    ...
    if np.any(k < 0) or np.any(k >= alpha.shape[1]):
        raise ValueError("k contains out-of-range survival interval indices.")
...
def _validate_treatment_shapes(
    *,
    theta: np.ndarray,
    gamma: np.ndarray,
    u: np.ndarray,
    ...
    if gamma.shape[0] != n_draws or u.shape[0] != n_draws:
        raise ValueError("All treatment sample blocks must have the same number of draws.")
    ...
    if np.any(k < 0) or np.any(k >= gamma.shape[1]):
        raise ValueError("k contains out-of-range treatment interval indices.")
