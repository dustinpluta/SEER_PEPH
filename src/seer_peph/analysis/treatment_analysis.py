...
class TreatmentAnalysisConfig:
    input_path: str
    out_dir: str = "artifacts/standalone_treatment_analysis"
    ...
    # Treatment covariates (post-encoding names, before canonicalization)
...
def _write_treatment_extractions(fit, out_dir: Path) -> None:
    ttt = extract_treatment_effects(fit, include_draws=False)
    spatial = extract_spatial_fields(fit, include_draws=False)

    if "theta" in ttt:
        ttt["theta"].to_csv(out_dir / "treatment_theta_summary.csv", index=False)
    if "gamma" in ttt:
        ttt["gamma"].to_csv(out_dir / "treatment_gamma_summary.csv", index=False)

    if "field" in spatial:
        spatial["field"].to_csv(out_dir / "treatment_spatial_field_summary.csv", index=False)
...
def _encode_like_prep(
    ...
    *,
    ...
    plus any modeled covariates named in ttt_x_cols.
    """
    df = wide.copy()
