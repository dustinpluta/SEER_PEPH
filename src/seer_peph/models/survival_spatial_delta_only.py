...
def model(data: Mapping[str, Any]) -> None:
    """
    Spatial survival model with

      - interval-specific log-baseline hazards alpha[k]
      - survival fixed effects beta
      - post-treatment step effects delta_post[k_post]
      - BYM2 spatial frailty u[a]

    and NO separate beta_td term.

    For treated rows, delta_post[0] is the acute treatment effect and later
    delta_post entries are the effects in later post-treatment intervals.
    For untreated rows, the treatment-history term is zero because it is
    multiplied by treated_td.
    """
    ...
    # Plain delta-only independent priors
    delta_post = numpyro.sample(
        "delta_post",
        dist.Normal(0.0, 0.5).expand([K_post]),
    )
